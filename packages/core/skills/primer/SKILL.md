...
3. Split the Unit's knowledge into `ASSUMES` and `TEACHES`. `ASSUMES` contains prerequisite Nodes, including Nodes absent from `nodes.json`. `TEACHES` contains only Nodes established by this Unit. Give each new Node a stable slug, title, one or two sentence summary, `dependsOn`, and `taughtAt`. For video, `taughtAt` is the end of the Node's teaching, in seconds. Find the last cue that still teaches the Node, including its examples, elaboration, and answers to learner questions. Use that cue's end time, which is where the lecturer moves on to the next topic. Never use the cue where the Node is introduced, named, or first stated. For a document, use the page where the Node's teaching ends.
