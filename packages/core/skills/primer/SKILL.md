...
3. Split the Unit's knowledge into `ASSUMES` and `TEACHES`. `ASSUMES` contains prerequisite Nodes, including Nodes absent from `nodes.json`. `TEACHES` contains only Nodes established by this Unit. Give each new Node a stable slug, title, one or two sentence summary, `dependsOn`, and `taughtAt`. For video, `taughtAt` is the end of the Node's teaching, in seconds. Find the last cue that still teaches the Node, including its examples and elaboration. Use that cue's end time, which is where the lecturer closes the topic and moves on. A later question and answer exchange that only revisits the Node does not extend it. Never use the cue where the Node is introduced, named, or first stated. For a document, use the page where the Node's teaching ends.
...
7. Build one Quiz plan for this Unit. For every taught Node, create several source-grounded choice Questions across both Tiers, with at least one `recall` and one `application`. Order each Node's Questions as a ladder: recall of the fact, then step Questions, then application to a new case. A step Question tests one link of the reasoning, such as which assumption allows a given step, or what fails when that assumption is dropped. Step Questions are `application` Tier. For a Node that is a derivation or an argument, write one step Question per assumption and one per step, including every assumption the derivation needs. Every Question names exactly one Node. Question ids are unique across the whole Course, so prefix each one with the Unit id. Keep unseen Questions in the pool for Spaced review.
8. Place one Pre-question per major segment. For a `youtube-video` Unit, require its `location` in the gap before that segment starts, after the previous segment's last cue ends. A Pre-question primes content the learner has not heard yet, so a timestamp at or after the segment's first cue interrupts the explanation it was meant to prime. Put Pause quizzes at Node boundaries using `taughtAt`, never merely by elapsed time, with one or two Questions each. A Pause fires the moment playback reaches its timestamp, so place it at the latest `taughtAt` of its Nodes, on a cue where the lecturer closes the topic, and never inside a sentence or a question and answer exchange. Sort placements by timestamp. The server rejects a video placement that runs backwards or lands less than 30 seconds after the previous one. When two placements crowd each other, move the Pre-question later to the next lull that still precedes the cue teaching its answer, or drop it. Mix both Tiers across the Pause quizzes, and place step Questions there, so the Recap quiz is never the first application Question on a Node. Make the Recap quiz five to eight choice Questions, application heavy, with currently unanswered and missed Questions first, followed by one Explain-back with a grading rubric. The Explain-back asks for one thing. Never join two tasks in one prompt. Every part of its rubric must already be tested by a choice Question placed earlier in the plan, so the Explain-back assembles pieces the learner has met and never introduces a new demand. If a rubric part has no such Question, add the Question or cut the part. Across choice Questions shown by placements, target about 60 percent recall and 40 percent application. Build every choice set with the `teach` skill's "Writing quiz options" construction procedure. Vary the position of the correct option across Questions. A fixed position is a tell.
9. Give the Source material and draft Quiz plan to a `researcher` pass. Check every prompt, correct option, explanation, and rubric claim against the Source material. Check every video placement timestamp the same way. Quote the cue at that second and the cue on each side of it, then confirm each Pre-question sits before its Node's first teaching cue and each Pause sits after its Nodes' teaching closes. Move any placement that fails this check. Fix or drop anything untraceable. Keep trace notes outside the payload because the schema rejects extra fields.
