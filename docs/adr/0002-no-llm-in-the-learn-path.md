...
While the learner works through a unit, nothing waits on a model. The agent writes each unit's quiz plan ahead of time, during that unit's primer, with a pool of questions per node across difficulty tiers. At learn time the server picks a tier with a plain rule that steers the rolling success rate toward about 85 percent. Difficulty adapts inside a unit through the pool, and between units through the next primer, which sees the fresh outcomes.
